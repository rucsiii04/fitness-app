...
function DateTimePicker({ value, onChange }) {
  ...
  const [calYear, setCalYear]   = React.useState(() => selDate ? parseInt(selDate.slice(0,4)) : now.getFullYear());
  const [calMonth, setCalMonth] = React.useState(() => selDate ? parseInt(selDate.slice(5,7))-1 : now.getMonth());

  function iso(date, h, m) {
    return `${date}T${String(h).padStart(2,"0")}:${String(m).padStart(2,"0")}`;
  }
  function pickDate(dk) { onChange(iso(dk, selHour, selMin)); setCalOpen(false); }
  function setHour(h)   { if (selDate) onChange(iso(selDate, ((h % 24) + 24) % 24, selMin)); }
  function setMin(m)    { if (selDate) onChange(iso(selDate, selHour, ((m % 60) + 60) % 60)); }
  function prevMonth()  { if (calMonth === 0) { setCalYear(y=>y-1); setCalMonth(11); } else setCalMonth(m=>m-1); }
  function nextMonth()  { if (calMonth === 11) { setCalYear(y=>y+1); setCalMonth(0); } else setCalMonth(m=>m+1); }

  const dispDate = selDate
    ? (() => { const [y,mo,d] = selDate.split("-").map(Number); return `${d} ${MONTHS_SHORT[mo-1]} ${y}`; })()
    : "Alege data";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <div style={{ position: "relative" }}>
        {calOpen && (
          <div style={{ position: "fixed", inset: 0, zIndex: 49 }} onClick={() => setCalOpen(false)} />
        )}
        <button
          type="button"
        ...
        >
          ...
          {selDate && (
            <span
              ...
              style={{ color: "var(--text-dim)", cursor: "pointer", fontSize: 18, lineHeight: 1, paddingLeft: 4 }}
            >×</span>
          )}
        </button>
          ...
          <div style={{
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom: 10 }}>
              <button type="button" onClick={prevMonth}
                style={{ background:"none", border:"none", color:"var(--accent)", cursor:"pointer", fontSize: 22, lineHeight:1, padding:"0 6px" }}>‹</button>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", marginBottom: 4 }}>
              {DAY_HDR.map(d => (
                <div key={d} style={{ textAlign:"center", fontSize: 9, fontWeight: 700, color:"var(--text-dim)", letterSpacing:.5, paddingBottom: 6 }}>{d}</div>
              ))}
            </div>
            ...
            {weeks.map((week, wi) => (
              <div key={wi} style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)" }}>
                {week.map((day, di) => {
                  ...
                  return (
                    <button key={di} type="button" onClick={() => !isPast && pickDate(dk)}
                      style={{
      ...
      {selDate && (
        <div style={{
          ...
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap: 3 }}>
            <button type="button" onClick={() => setHour(selHour + 1)} style={spinBtn}>▲</button>
            <span style={{ fontSize: 20, fontWeight: 700, color:"var(--text)", minWidth: 32, textAlign:"center" }}>
          ...
          {/* Minute */}
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap: 3 }}>
            <button type="button" onClick={() => setMin(selMin + 5)} style={spinBtn}>▲</button>
            <span style={{ fontSize: 20, fontWeight: 700, color:"var(--text)", minWidth: 32, textAlign:"center" }}>
...
export default function AdminSettings() {
  ...
  const load = () => {
    ...
    if (gymId) {
      proms.push(
        getAdminFreezeStatus(gymId).catch(() => ({ data: { frozen_count: 0 } }))
      );
    }
    Promise.all(proms)
      .then(([gymsRes, freezeRes]) => {
        ...
        if (g) {
          ...
          const loadedEndAt = g.alert_expires_at ? new Date(g.alert_expires_at).toISOString().slice(0, 16) : "";
          setAlertForm({
  ...
  return (
    <>
            ...
            {section === "Notificări" && (
              <>
                <div
                  style={{
                    padding: 14,
                    background: activeAlert ? "rgba(255,115,81,.08)" : "var(--surface-2)",
                ...
                >
                  <I.bell
                    width={16}
                    height={16}
                    style={{ color: activeAlert ? "var(--coral)" : "var(--text-dim)", marginTop: 2, flexShrink: 0 }}
                  />
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: activeAlert ? "var(--coral)" : "var(--text-dim)", marginBottom: 4 }}>
                      ...
                      <>
                        ...
                        {gym.alert_expires_at && (
                          <div className="mono" style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 4 }}>
                            Expiră la {new Date(gym.alert_expires_at).toLocaleString("ro-RO", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}
                          </div>
                        )}
                        {!gym.alert_expires_at && (
                          <div className="mono" style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 4 }}>
                ...
                <Panel title="Alertă / Notificare" eyebrow="Vizibilă pentru toți membrii">
                  <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                    <Field label="Mesaj">
                      <textarea
                      ...
                      <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", userSelect: "none" }}>
                        <div
                          onClick={() => {
                            ...
                            setHasExpiry(next);
                            if (!next) setAlertForm((f) => ({ ...f, end_at: "" }));
                          }}
                          style={{
                            width: 36, height: 20, borderRadius: 10, flexShrink: 0,
                            background: hasExpiry ? "var(--accent)" : "var(--surface-3)",
                        ...
                        >
                          <div style={{
                            ...
                            background: hasExpiry ? "var(--bg)" : "var(--text-dim)",
                            transition: "left .2s",
                          }} />
                        </div>
                      ...
                      {hasExpiry && (
                        <div style={{ marginTop: 10 }}>
                          <DateTimePicker
                            ...
                            onChange={(v) => setAlertForm((f) => ({ ...f, end_at: v }))}
                          />
                ...
                <Panel title="Stare abonamente" eyebrow="Închidere temporară sală">
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <div style={{
                        width: 36, height: 36, borderRadius: 8, flexShrink: 0,
                        background: frozenCount > 0 ? "rgba(147,197,253,.15)" : "var(--surface-2)",
                        display: "flex", alignItems: "center", justifyContent: "center",
                      }}>
                        {frozenCount > 0
                          ? <I.snowflake width={16} height={16} style={{ color: "#93c5fd" }} />
                          : <I.building width={16} height={16} style={{ color: "var(--text-dim)" }} />
                        }
                      </div>
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: frozenCount > 0 ? "#93c5fd" : "var(--text)", marginBottom: 3 }}>
                          {frozenCount > 0
                        ...
                        <div style={{ fontSize: 12, color: "var(--text-dim)" }}>
                          {frozenCount > 0
                            ? "Membrii nu pierd zile. Dezgheață la redeschidere."
                            : "Îngheață toate abonamentele active la închiderea sălii."}
                        </div>
                      </div>
                    </div>
                    {frozenCount > 0 ? (
                      <Btn variant="outline" size="sm" onClick={() => setUnfreezeOpen(true)} disabled={freezeWorking || !gymId}>
                        Redeschide sala
                      </Btn>
                    ) : (
                      <Btn variant="outline" size="sm" onClick={() => setFreezeOpen(true)} disabled={freezeWorking || !gymId}>
                        Îngheață abonamente
                      </Btn>
                    )}
                  </div>
      ...
      <Modal open={freezeOpen} onClose={() => setFreezeOpen(false)} title="Îngheață abonamentele sălii">
        <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ fontSize: 14, color: "var(--text-dim)", lineHeight: 1.6 }}>
            Toate abonamentele active vor fi înghețate. Membrii nu vor pierde zile cât timp sala este închisă.
            Datele de expirare vor fi extinse automat la redeschidere.
          </div>
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
            <Btn variant="outline" onClick={() => setFreezeOpen(false)} disabled={freezeWorking}>Anulează</Btn>
            <Btn variant="primary" onClick={handleAdminFreeze} disabled={freezeWorking} icon={<I.snowflake />}>
      ...
      <Modal open={unfreezeOpen} onClose={() => setUnfreezeOpen(false)} title="Redeschide sala & dezgheață abonamentele">
        <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ fontSize: 14, color: "var(--text-dim)", lineHeight: 1.6 }}>
            {frozenCount} abonament{frozenCount !== 1 ? "e" : ""} vor fi reactivat{frozenCount !== 1 ? "e" : ""}.
            Fiecare dată de expirare va fi extinsă cu numărul exact de zile cât sala a fost închisă.
            Abonamentele puse în pauză de membri rămân neschimbate.
          </div>
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
            <Btn variant="outline" onClick={() => setUnfreezeOpen(false)} disabled={freezeWorking}>Anulează</Btn>
            <Btn variant="primary" onClick={handleAdminUnfreeze} disabled={freezeWorking} icon={<I.unlock />}>
