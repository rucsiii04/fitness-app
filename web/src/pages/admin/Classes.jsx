...
const SELECT_STYLE = {
  ...
  borderRadius: 10, color: "var(--text)", fontSize: 14, width: "100%",
};
...
const STATUS_TONE = { scheduled: "outline", ongoing: "accent", completed: "muted", cancelled: "red" };

const EMPTY_SESSION_FORM = {
  class_type_id: "", trainer_id: "", start_datetime: "", end_datetime: "", max_participants: 20,
};
const EMPTY_TYPE_FORM = { name: "", description: "", difficulty_level: "beginner" };
...
export default function AdminClasses() {
  ...
  const dayKey = localDate(weekDates[selectedDay]);
  const daySessions = sessions
    .filter((s) => localDate(s.start_datetime) === dayKey)
  ...
  const handleCreateType = async (e) => {
    ...
    try {
      ...
      getClassTypesByGym(gymId).then((r) => setClassTypes(r.data)).catch(() => {});
    } catch (err) {
  ...
  const handleCancelSession = async (sessionId) => {
    if (!window.confirm("Cancel this class? All enrolled members will be notified.")) return;
    try {
  ...
  return (
    <>
      <TopBar
        title="Classes"
        eyebrow={`Schedule · ${sessions.length} sessions this week`}
        actions={
          <div style={{ display: "flex", gap: 8 }}>
            <Btn variant="outline" icon={<I.plus />} onClick={() => setTypeOpen(true)}>
              New class type
            </Btn>
            <Btn variant="primary" icon={<I.plus />} onClick={() => setCreateOpen(true)}>
              Create class
            </Btn>
          </div>
        }
      ...
      <div style={{
        padding: "20px 32px", display: "flex", gap: 8,
        ...
        {weekDates.map((d, i) => (
          <button
            ...
            style={{
              ...
              background: i === selectedDay ? "var(--accent)" : "var(--surface)",
              color: i === selectedDay ? "var(--accent-ink)" : "var(--text)",
              border: `1px solid ${i === selectedDay ? "var(--accent)" : "var(--border)"}`,
              textAlign: "left",
            }}
          >
            <div className="mono" style={{
              fontSize: 10, fontWeight: 700,
              color: i === selectedDay ? "var(--accent-ink)" : "var(--text-dim)",
            }}>
              {DAYS[i]}
            </div>
            <div className="display" style={{ fontSize: 22, marginTop: 3 }}>{d.getDate()}</div>
            <div className="mono" style={{
              fontSize: 9, opacity: 0.7, marginTop: 4,
              color: i === selectedDay ? "var(--accent-ink)" : "var(--text-dim)",
            }}>
              {sessionCount(d)} classes
            </div>
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: "20px 32px" }}>
        ...
        {loading && (
          <div style={{ padding: "64px 0", textAlign: "center", color: "var(--text-dim)", fontFamily: "var(--mono)", fontSize: 12 }}>
            Loading...
          </div>
        )}
        {!loading && gymId && daySessions.length === 0 && (
          <div style={{ padding: "64px 0", textAlign: "center", color: "var(--text-dim)" }}>
            <I.calendar width={32} height={32} style={{ opacity: 0.3, marginBottom: 12 }} />
            ...
            {daySessions.map((c) => {
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <Progress value={fill * 100} color={fill >= 1 ? "var(--coral)" : "var(--accent)"} />
                    <span className="mono" style={{ fontSize: 11, color: "var(--text-muted)", width: 44 }}>
                      {c.confirmed_count || 0}/{c.max_participants}
                    </span>
                  </div>

                  <Pill tone={STATUS_TONE[c.status] || "outline"}>{c.status}</Pill>
        ...
        >
          <Field label="Class Type">
            <select
              ...
              onChange={(e) => setSessionForm((f) => ({ ...f, class_type_id: e.target.value }))}
              required
              style={SELECT_STYLE}
            >
              <option value="">— Select class type —</option>
              {classTypes.map((t) => (
                <option key={t.class_type_id} value={t.class_type_id}>{t.name}</option>
              ))}
            </select>
            {classTypes.length === 0 && (
              <div style={{ fontSize: 11, color: "var(--text-dim)", marginTop: 4 }}>
                ...
                <button
                  type="button"
                  onClick={() => { setCreateOpen(false); setTypeOpen(true); }}
          ...
          <Field label="Trainer">
            <select
              ...
              onChange={(e) => setSessionForm((f) => ({ ...f, trainer_id: e.target.value }))}
              required
              style={SELECT_STYLE}
            >
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <Field label="Start">
              <Input
                ...
                onChange={(e) => setSessionForm((f) => ({ ...f, start_datetime: e.target.value }))}
                required
              />
            </Field>
            <Field label="End">
              <Input
                ...
                onChange={(e) => setSessionForm((f) => ({ ...f, end_datetime: e.target.value }))}
                required
              />
            </Field>
          </div>
      ...
      <Modal open={typeOpen} onClose={() => setTypeOpen(false)} title="New Class Type">
        <form
        ...
        >
          ...
          <Field label="Difficulty">
            <select
              ...
              onChange={(e) => setTypeForm((f) => ({ ...f, difficulty_level: e.target.value }))}
              style={SELECT_STYLE}
            >
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
            <Btn variant="outline" type="button" onClick={() => setTypeOpen(false)}>Cancel</Btn>
            <Btn variant="primary" type="submit">Create</Btn>
          </div>
        </form>
      </Modal>

      <Modal open={!!enrollOpen} onClose={() => setEnrollOpen(null)} title="Class Roster">
          ...
          {enrollOpen && (
            <div style={{
              ...
              <div className="mono" style={{ fontSize: 11, color: "var(--text-dim)", marginTop: 2 }}>
                {fmtTime(enrollOpen.start_datetime)} · {durationMin(enrollOpen.start_datetime, enrollOpen.end_datetime)} min
              </div>
            </div>
          )}
            ...
            {enrollments.length === 0 ? (
              ...
              enrollments.map((e) => {
                const name = e.Client
                  ...
                  : `Member #${e.client_id}`;
                const ENROLL_TONE = {
                  confirmed: "muted", attended: "green",
                  no_show: "red", waiting_list: "coral", cancelled: "muted",
                };
                return (
                  <div
                    key={e.enrollment_id}
                  ...
                  >
                    ...
                    <Pill tone={ENROLL_TONE[e.status] || "muted"}>{e.status}</Pill>
                  </div>
                );
              })
            )}
